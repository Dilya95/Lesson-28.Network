...
### Таблица подсетей (как в методичке + проверка)
...
**Свободные подсети** (внутри 192.168.0.0/24 и 192.168.255.0/24):

- 192.168.0.16/28  
- 192.168.0.48/28  
- 192.168.0.128/25  
- 192.168.255.12/30 (и дальше вверх)  
- 192.168.255.16/28, /29, /27 и т.д.

**Ошибок разбиения нет** — все подсети не пересекаются.
